...
from permissions.permissions import admin_required, sales_required, accountant_required, allowed_users
...
class ProcessTransferCartView(LoginRequiredMixin, View):
    ...
    def deduct_inventory(self, transfer_item):
        ...
        branch_inventory = Inventory.objects.get(product__name=transfer_item.product,
...
def inventory(request):
    ...
    if product_name:
        logger.info(
            f'{list(Inventory.objects.filter(product__name=product_name, branch=request.user.branch).values())}')
        return JsonResponse(
            list(Inventory.objects.filter(product__name=product_name, branch=request.user.branch).values()), safe=False)
...
def inventory_index(request):
    ...
    if 'download' and 'excel' in request.GET:
        ...
        for branch in branches:
            ...
            categories = Inventory.objects.filter(branch=request.user.branch).values_list('product__category__name',
                                                                                          flat=True).distinct()
...
def edit_inventory(request, product_name):
    ...
    return render(request, 'inventory/inventory_form.html',
                  {'product': inv_product, 'title': f'Edit >>> {inv_product.product.name}'})
...
def inventory_transfers(request):
    ...
    return render(request, 'inventory/transfers.html',
                  {'transfers': transfers, 'search_query': q, 'form': form, 'transfer_items': transfer_items})
        ...
        try:
            ...
            if request.POST['received'] == 'true':
                ...
                if Inventory.objects.filter(product=branch_transfer.product, branch=request.user.branch).exists():
                    existing_inventory = Inventory.objects.get(product=branch_transfer.product,
                                                               branch=request.user.branch)
...
def inventory_pdf(request):
    ...
    inventory = get_list_or_404(Inventory, branch=request.user.branch.id,
                                product__category__name=category) if category else get_list_or_404(Inventory,
...
def reorder_from_notifications(request):
    if request.method == 'GET':
        notifications = StockNotifications.objects.filter(inventory__branch=request.user.branch,
                                                          inventory__reorder=False).values(
            'quantity',
